function [ Y, T ] = odeAM3( f, Y_0, ts, te, step )
    h = step;
    N = (te-ts)/h + 1;
    Y = zeros(2, N);
    % Define numerical methods
    EE = @(y_n) y_n + h.*f(y_n);
    AM1 = @(y_n, y_n1) y_n + (1/2)*h.*(f(y_n1) + f(y_n));
    AM2 = @(y_n, y_n1, y_n2) y_n1 + (1/12)*h.*(5*f(y_n2) + 8*f(y_n1) - 5*f(y_n));
    AM3 = @(y_n, y_n1, y_n2, y_n3) y_n1 + (1/24)*h.*(9*f(y_n2) + 19*f(y_n2) - 5*f(y_n1) + f(y_n));
    ...
    for i = 3:N
        ...
        while true
            ...
            % Refine guess with fixed point iteration using Adam Moulton-3
            c2 = AM3(Y(:,i-2), Y(:,i-1), c1);
